...
#            p0_b = norm_b.getVal() / (int_b*lumi)*float(options.bin_width)
            ...
            function_fit = TF1("background", "[0] * " + function_expression("bkg_norm[1]",'function_expression'), range_of_fit_left, range_of_fit_right)
            for i,n in enumerate(parameters_name):
                function_fit.SetParName(i+1,n)
            for i,p in enumerate(parameters):
                function_fit.SetParameter(p.GetTitle(), p.getVal())
                print p.GetTitle(), "=", p.getVal()
            function_fit.SetParameter(0,1)
            function_fit.SetParName(0,'p0')
            integ = function_fit.Integral(massMin, massMax)
            function_fit.SetParameter(0,norm_b.getVal() / integ  * float(options.bin_width))
            #SetParameter(1,p0_b)
...
def simple_fit(hist, fexpr, function_tag):
    fitFunction = ""
    function_fit = TF1(function_name, "[0] * " + fexpr, range_of_fit_left, range_of_fit_right)
    ...
    if path == "":
        ...
        for m in mass:
            ...
            for file_ in pathlist:
                if ('rootfile_list_VectorDiJet1Jet_'+m) in file_ and 'reduced_skim.root' in file_:
                    rootTree[m].Add(directory+"/"+file_)
                    fileName = file_
