...
varName = "dphi"
varTex = "Delta#phi (jj)"
#values = [round(x/10.,1) for x in range(0,32,1)]
#values = [round(x/10.,1) for x in range(0,32,1)]
#values = [round(x/10.,1) for x in range(0,32,1)]
values = [0,1.57075,2.0943333333333336,2.356125]
selection = "isr_pt>70 && HLT_CaloScoutingHT250 && abs(dijet_deta)<1.1  && abs(TVector2::Phi_mpi_pi(jet1_phi-jet2_phi))>%f && abs(jet1_eta)<2.5 && abs(jet2_eta)<2.5 && abs(isr_eta)<2.5 && HLT_CaloScoutingHT250 "
#'''
...
dataFileName = "../data_trig_eff_eta2.5_skim.root"
...
signalGoodMatch = {}
for value in values:
    data[value] = getHisto(dataFileName)
    for mass in masses:
        signal[(value,mass)] = getHisto(signalFileName%(mass))
        signalGoodMatch[(value,mass)] = getHisto(signalFileName%(mass), selection + " && (sqrt(TVector2::Phi_mpi_pi(jet1_phi-jet1MC_phi)**2 + (jet1_eta-jet1MC_eta)**2)<0.4 && sqrt(TVector2::Phi_mpi_pi(jet2_phi-jet2MC_phi)**2 + (jet2_eta-jet2MC_eta)**2)<0.4) ")
    print(data[value])
    ...
    for mass in masses:
        ...
        for i,value in enumerate(values):
            ...
            if i==0:
                ...
                signals[(value,mass)].GetYaxis().SetRangeUser(1E0,1E3)
...
for matching in [True,False]:
    ...
    c1.Modified()
    if matching:
        c1.SaveAs("plots_dphi_%s_sig_matching.png"%varName)
        c1.SaveAs("plots_dphi_%s_sig_matching.pdf"%varName)
    else:
        c1.SaveAs("plots_dphi_%s_sig.png"%varName)
        c1.SaveAs("plots_dphi_%s_sig.pdf"%varName)
