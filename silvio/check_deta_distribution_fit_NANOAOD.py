...
#bins = [60,70,80,90]
#bins = range(8,13)
bins = [i*1./10. for i in range(92,105,2)]
#bins = range(7,13)
#bins = range(8,15)
...
denTrigger = "abs(Jet_eta[1])<2.5 && abs(Jet_eta[0])<2.5  && Jet_pt[2]>=0"
#denTrigger = "abs(jet2_eta)<2.5 && abs(jet1_eta)<2.5 && isr_pt>=70"
...
varX = "abs(Jet_eta[0] - Jet_eta[1])"

#varX_nbins,   varX_min,  varX_max = 100,-5,5
varX_nbins,   varX_min,  varX_max = 100,0,4
...
for i in range(len(bins)-1):
    preselect = denTrigger + "&& log(Jet_pt[0]*Jet_pt[1])>%f && log(Jet_pt[0]*Jet_pt[1])<%f"%(bins[i],bins[i+1]) #
...
#c2.SetLogy()
leg.Draw()
c2.SaveAs("deta2plotcheck.png")
c2.SaveAs("deta2plotcheck.pdf")
