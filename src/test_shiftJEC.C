...
/*std::string L1Path = "data/Spring16_23Sep2016V2_MC/Spring16_23Sep2016V2_MC_L1FastJet_AK4PFchs.txt";
std::string L2Path = "data/Spring16_23Sep2016V2_MC/Spring16_23Sep2016V2_MC_L2Relative_AK4PFchs.txt";
std::string L3Path = "data/Spring16_23Sep2016V2_MC/Spring16_23Sep2016V2_MC_L3Absolute_AK4PFchs.txt";*/
 ...
 //uncertainty
    unc = new JetCorrectionUncertainty("data/Spring16_23Sep2016V1_DATA/Spring16_23Sep2016BCDV1_DATA_Uncertainty_AK4PFchs.txt");
   // unc = new JetCorrectionUncertainty("data/Spring16_25nsV6_MC/Spring16_25nsV6_MC_Uncertainty_AK4PFchs.txt"); 
//unc = new JetCorrectionUncertainty("data/Fall17_17Nov2017_V6_DATA/Fall17_17Nov2017D_V6_DATA_Uncertainty_AK4PFchs.txt");
 ...
 // uncertainty sources//
/*
//    const char* srcnames[] =  {"SubTotalRelative"};
//    const char* srcnames[] =  {"SubTotalPt"};
//    const char* srcnames[] =  {"SubTotalAbsolute"};
//    const char* srcnames[] =  {"SubTotalScale"};
//    const char* srcnames[] =  {"SubTotalPileUp"};
     const char* srcnames[] =  {"SubTotalMC"};
    const int nsrc = sizeof(srcnames)/sizeof(char*)-1;
    std::vector<JetCorrectionUncertainty*> vsrc(nsrc);

   cout << " nsrc is " << nsrc << endl;
   for (int isrc = 0; isrc <=nsrc; isrc++) {  
    const char *name = srcnames[isrc];
    cout << " name is " << name << endl;
    JetCorrectorParameters *p = new JetCorrectorParameters("data/Spring16_25nsV6_DATA/Spring16_25nsV6_DATA_UncertaintySources_AK4PFchs.txt", name);
    JetCorrectionUncertainty *uncsrc = new JetCorrectionUncertainty(*p);
    unc = uncsrc;
    //cout << " here 1 " << unc <<  endl;
  } // for isrc
    int the_src = 0;
*/ // end of accesing uncertainty sources
	 ...
	 for(size_t j=0; j<no_jets_ak4; ++j)
	   {
	     ...
	     unc->setJetPt(jetPtAK4->at(j)/jetJecAK4->at(j)*correction); // undo the existing JEC and apply the current one
	     double uncertainty = unc->getUncertainty(true);
	     jecUncertainty.push_back(uncertainty); 
             //cout << " uncertainty is " << uncertainty << " eta is " <<  jetEtaAK4->at(j) <<  " jet pt is  " << jetPtAK4->at(j) <<endl; 
	     if( int(getPreCutValue1("shiftJECs"))!=0){
	       correction = correction + getPreCutValue2("shiftJECs")*uncertainty*correction;	       
	     }// Value1 of shiftJECs 1 Value2 of shiftJECs +1 gia +1 sigma -1 gia -1 sigma
		  ...
		  && (jecFactors[sortedJetIdx[1]]/jetJecAK4->at(sortedJetIdx[1]))*jetPtAK4->at(sortedJetIdx[1]) > getPreCutValue1("pt1Cut"))
		 {
		   ...
		   for(Long64_t ijet=0; ijet<no_jets_ak4; ijet++)
		     { //jet loop for ak4
		       ...
		       if(fabs(jetEtaAK4->at(sortedJetIdx[ijet])) < getPreCutValue1("jetFidRegion") 
			  && idTAK4->at(sortedJetIdx[ijet]) == getPreCutValue1("tightJetID") &&  jetCemfAK4->at(sortedJetIdx[0]) < 0.8
			  && (jecFactors[sortedJetIdx[ijet]]/jetJecAK4->at(sortedJetIdx[ijet]))*jetPtAK4->at(sortedJetIdx[ijet]) > getPreCutValue1("ptCut"))
			 {
			   TLorentzVector currentJet;
