...
TH1F* h_CEMF_test1 = new TH1F("h_CEMF_test1", "h_CEMF_test1", 50, 0, 1.0);
 ...
 //for (Long64_t jentry=0; jentry<nentries;jentry=jentry+10) {   //takes every 10th en
     ...
     else
     {
       ...
       if(no_jets_ak4>=2)
	 {
	   if(fabs(jetEtaAK4->at(sortedJetIdx[0])) < getPreCutValue1("jetFidRegion")  // && idTAK4->at(sortedJetIdx[0]) == getPreCutValue1("tightJetID")
	      && (jecFactors[sortedJetIdx[0]]/jetJecAK4->at(sortedJetIdx[0]))*jetPtAK4->at(sortedJetIdx[sortedJetIdx[0]]) > getPreCutValue1("pt0Cut"))
	     {
	       if(fabs(jetEtaAK4->at(sortedJetIdx[1])) < getPreCutValue1("jetFidRegion") // && idTAK4->at(sortedJetIdx[1]) == getPreCutValue1("tightJetID")
