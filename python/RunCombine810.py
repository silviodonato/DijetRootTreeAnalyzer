    ...
    for box,lumi in zip(boxes,lumiFloat):
        ...
        for massPoint in massIterable(options.mass):
            ...
            if options.bayes:
                ...
                if len(boxes)==1:
                    exec_me('combine -M MarkovChainMC -H AsymptoticLimits %s/dijet_combine_%s_%s_lumi-%.3f_%s.txt -n %s_%s_lumi-%.3f_%s --tries 20 --proposal ortho --burnInSteps 200 --iteration 30000 --propHelperWidthRangeDivisor 10 %s %s %s %s'%(options.outDir,model,massPoint,lumi,box,model,massPoint,lumi,box,rRangeString,blindString,sysString,toyString),options.dryRun)
                ...
                if signif:
                    ...
                    if options.rMax>-1:
                        ...
                        rRangeStringList.append(rRangeString)
                    if len(boxes)==1:
                        exec_me('combine -M Significance --signif %s/dijet_combine_%s_%s_lumi-%.3f_%s.txt -n %s_%s_lumi-%.3f_%s %s %s'%(options.outDir,model,massPoint,lumi,box,model,massPoint,lumi,box,rRangeString,sysString),options.dryRun)
                        exec_me('mv higgsCombine%s_%s_lumi-%.3f_%s.Significance.mH120.root %s/'%(model,massPoint,lumi,box,options.outDir),options.dryRun)
                else:
                    rRangeString = ''
                    if options.rMax>-1:                
                        rRangeString =  '--setParameterRanges r=0,%f'%(options.rMax)                
                        rRangeStringList.append(rRangeString)
                    if len(boxes)==1:
                        #--minosAlgo bisection --newExpected false
                        exec_me('combine  -M AsymptoticLimits  --newExpected false %s/dijet_combine_%s_%s_lumi-%.3f_%s.txt -n %s_%s_lumi-%.3f_%s --cminDiscreteMinTol %f --cminDefaultMinimizerStrategy %i %s --saveWorkspace %s %s'%(options.outDir,model,massPoint,lumi,box,model,massPoint,lumi,box,options.min_tol,options.min_strat,rRangeString,blindString,sysString),options.dryRun)
                        exec_me('mv higgsCombine%s_%s_lumi-%.3f_%s.AsymptoticLimits.mH120.root %s/'%(model,massPoint,lumi,box,options.outDir),options.dryRun)
    if len(boxes)>1:
        lumiTotal = sum(lumiFloat)
        ...
        if options.bayes:
            rRangeStringListMod = [rRangeString.replace('--setParameterRanges ','') for rRangeString in rRangeStringList ]
            paramRangeList = []
            ...
            exec_me('combine -M MarkovChainMC -H AsymptoticLimits %s/dijet_combine_%s_%s_lumi-%.3f_%s.txt -n %s_%s_lumi-%.3f_%s --tries 30 --proposal ortho --burnInSteps 1000 --iteration 40000 --propHelperWidthRangeDivisor 10 %s %s %s %s'%(options.outDir,model,massPoint,lumiTotal,options.box,model,massPoint,lumiTotal,options.box,rRangeStringTotal,blindString,sysStringTotal,toyString),options.dryRun)
            ...
            if signif:
                ...
                if options.rMax>-1:
                    rRangeString = '--setParameterRanges r=0,%f'%(options.rMax)
                exec_me('combine -M Significance --signif %s/dijet_combine_%s_%s_lumi-%.3f_%s.txt -n %s_%s_lumi-%.3f_%s %s %s'%(options.outDir,model,massPoint,lumiTotal,options.box,model,massPoint,lumiTotal,options.box,rRangeString,sysString),options.dryRun)
                exec_me('mv higgsCombine%s_%s_lumi-%.3f_%s.Significance.mH120.root %s/'%(model,massPoint,lumiTotal,options.box,options.outDir),options.dryRun)
            else:
                rRangeString = ''
                if options.rMax>-1:                
                    rRangeString =  '--setParameterRanges r=0,%f'%(options.rMax)
                exec_me('combine  --minosAlgo bisection -M AsymptoticLimits  --newExpected false %s/dijet_combine_%s_%s_lumi-%.3f_%s.txt -n %s_%s_lumi-%.3f_%s --cminDiscreteMinTol %f --cminDefaultMinimizerStrategy %i %s --saveWorkspace %s %s'%(options.outDir,model,massPoint,lumiTotal,options.box,model,massPoint,lumiTotal,options.box,options.min_tol,options.min_strat,rRangeString,blindString,sysString),options.dryRun)
                exec_me('mv higgsCombine%s_%s_lumi-%.3f_%s.AsymptoticLimits.mH120.root %s/'%(model,massPoint,lumiTotal,options.box,options.outDir),options.dryRun)
...
if __name__ == '__main__':
    ...
    parser.add_option('--min-tol',dest="min_tol",default=0.10000000000000001,type="float",
                  help="minimizer tolerance (default = 0.10000000000000001)")
    parser.add_option('--min-strat',dest="min_strat",default=1,type="int",
                  help="minimizer strategy (default = 1)")
