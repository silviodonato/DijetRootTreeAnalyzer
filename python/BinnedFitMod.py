...
def calculateChi2AndFillResiduals(data_obs_TGraph_,background_hist_,hist_fit_residual_vsMass_,workspace_,options,prinToScreen_=0,effFit_=False):
...
def BinnedFitMod(options,args,configFile,cutValueDir,minRange):
    ...
    for f in args:
        if f.lower().endswith('.root'):
            ...
            if type(histoName) is type(""):
                obj = rootFile.Get(histoName)
                if issubclass(obj.__class__, rt.TH1) or issubclass(obj.__class__, rt.TH1):
                    myTH1 = obj
            elif type(histoName) is type([]):
                for h in histoName:
                    obj = rootFile.Get(h)
                    if issubclass(obj.__class__, rt.TH1) or issubclass(obj.__class__, rt.TH1):
                        if myTH1 == None: myTH1 = obj
                        else: myTH1.Add(obj,1)
    ...
    if options.triggerDataFile is not None:
        ...
        if 'triggerData' not in names:
            ...
            triggerOutFile = rt.TFile.Open(optionsOutdir+"/triggerData.root","recreate")
            triggerOutFile.cd()
            triggerData.Write()
    ...
    if noFit and options.inputFitFile is not None:
        ...
        if not options.doSimultaneousFit:
            ...
            if options.doSpectrumFit:
                ...
                corrCanvas.Print(optionsOutdir+'/corrHist.pdf')
                corrCanvas.Print(optionsOutdir+'/corrHist.C')
            else:
                fr = rt.RooFitResult()

            if options.triggerDataFile is not None and options.doTriggerFit:
                corrHistEff.Draw('colztext')
                corrCanvas.Print(optionsOutdir+'/corrHistEff.pdf')
                corrCanvas.Print(optionsOutdir+'/corrHistEff.C')
        else:
            ...
            corrCanvas.SetRightMargin(0.15)
            corrHist.Draw('colztext')
            corrCanvas.Print(optionsOutdir+'/corrHistSim.pdf')
            corrCanvas.Print(optionsOutdir+'/corrHistSim.C')
    ...
    rootFile = rt.TFile.Open(optionsOutdir + '/' + 'Plots_%s'%box + '.root','recreate')
    rootDirectory = optionsOutdir.split("/")[-1]
    tdirectory = rootFile.GetDirectory(rootDirectory)
    if tdirectory==None:
        print "making directory"
        rootFile.mkdir(rootDirectory)
        print rootDirectory
        tdirectory = rootFile.GetDirectory(rootDirectory)
        tdirectory.Print('v')
    ...
    if options.triggerDataFile is not None:
        ...
        list_chi2AndNdf_background = calculateChi2AndFillResiduals(effGraph_coarse,h_eff,h_eff_residual_vs_mass,w,options,0,True)
        ...
        d.Print(optionsOutdir+"/eff_mjj_%s_%s.pdf"%(fitRegion.replace(',','_'),box))
        d.Print(optionsOutdir+"/eff_mjj_%s_%s.C"%(fitRegion.replace(',','_'),box))
        tdirectory.cd()
        d.Write()
    ...
    list_chi2AndNdf_background = calculateChi2AndFillResiduals(g_data,h_background,h_fit_residual_vs_mass,w,options,0)
    ...
    if not options.linearX:
        c.Print(optionsOutdir+"/fit_mjj_%s_%s.pdf"%(fitRegion.replace(',','_'),box))
        c.Print(optionsOutdir+"/fit_mjj_%s_%s.C"%(fitRegion.replace(',','_'),box))
    else:
        c.Print(optionsOutdir+"/fit_mjj_%s_%s_linearX.pdf"%(fitRegion.replace(',','_'),box))
        c.Print(optionsOutdir+"/fit_mjj_%s_%s_linearX.C"%(fitRegion.replace(',','_'),box))
