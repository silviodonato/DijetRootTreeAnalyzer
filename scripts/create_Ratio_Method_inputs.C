...
#include "TH1.h"
 ...
 void create_Ratio_Method_inputs(){
   ...
   double mcut_low = 2438.0;		//cut filling predictions
   double mcut_low_data = 1530.0;         //cut filling data histograms
   double Dratio_fit_start = 2438;
   double Dratio_fit_end = 8152;

  TFile *fMC = TFile::Open("root://cmseos.fnal.gov//eos/uscms/store/group/lpcjj/Ratio_Method_Prediction_2017_mjj2438.root","READ");
  
  TFile *fdata = TFile::Open("root://cmseos.fnal.gov//eos/uscms/store/group/lpcjj/small_tree_2016_2017_newJEC.root ","READ");
  ...
  TH1D *h_mjj,*h_mjj_high,*h_mjj_1GeVbin,*h_mjj_high_1GeVbin,*h_ratio_MC_1GeVbin;
   ...
   double massBoundaries[nMassBins+1] = {1, 3, 6, 10, 16, 23, 31, 40, 50, 61, 74, 88, 103, 119, 137, 156, 176, 197, 220, 244, 270, 296, 325,
    ...
    h_mjj_high_1GeVbin->Sumw2();
		...
		//creating smoothed 1GeV bin MC ratio
		for (int i=0; i<h_ratio_MC_1GeVbin->GetNbinsX(); i++){ 
	
			double mjj = h_ratio_MC_1GeVbin->GetBinCenter(i);
			double smooth_MC = h_ratio_MC->Interpolate(mjj);
		        if( mjj>mcut_low) h_ratio_MC_1GeVbin->SetBinContent(i,smooth_MC);
...
TTree *tree = (TTree*)(fdata->Get("t1"));
    ...
    TFile *fout = TFile::Open("inputs/Ratio_Method_inputs.root","RECREATE");
